...
class ColorAnalysis: ObservableObject {
    ...
    private func getContrastRate() {
        ...
        if (proportion >= 0 && proportion <= 1.3) {
            if contrast == 2 {
                self.season = .invernoProfundo
            }
            else if contrast == 1 {
                self.season = .invernoIntenso
            }
            else {
                self.season = .invernoPuro
            }
        }
        else if (proportion > 1.3 && proportion <= 1.5) {
            if contrast == 2 {
                self.season = .verãoPuro
            }
            else if contrast == 1 {
                self.season = .verãoClaro
            }
            else {
                self.season = .verãoSuave
            }
        }
        else if (proportion > 1.5 && proportion <= 1.7) {
            if contrast == 2 {
                self.season = .outonoProfundo
            }
            else if contrast == 1 {
                self.season = .outonoSuave
            }
            else {
                self.season = .outonoPuro
            }
        }
        else {
            if contrast == 2 {
                self.season = .primaveraIntensa
            }
            else if contrast == 1 {
                self.season = .primaveraClara
            }
            else {
                self.season = .primaveraPura
            }
        }
        print(self.season ?? .invernoPuro)
    }
