...
class ImageComparisonTableCellViewModel: ObservableObject {
    ...
    @Published var contrast: Contrast?
    ...
    func getRect(image: UIImage) {
        ...
        let request = VNDetectFaceLandmarksRequest(completionHandler: {(request: VNRequest, error: Error?) in
            ...
            self.createRectPath(faceContour: faceContourImage, image: image)
        ...
        let skinContrast = getContrast(skinColor: skinColor, eyeColor: eyeColor, eyebrowColor: eyebrowColor)
        self.contrast = skinContrast
    }
